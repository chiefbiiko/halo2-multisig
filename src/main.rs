...
use axiom_codec::{
    constants::{
        NUM_SUBQUERY_TYPES, USER_ADVICE_COLS, USER_FIXED_COLS,
        USER_INSTANCE_COLS, USER_LOOKUP_ADVICE_COLS,
    },
    types::{
        field_elements::AnySubqueryResult,
        native::{
            AccountSubquery, HeaderSubquery, StorageSubquery, SubqueryResult,
            SubqueryType,
        },
    },
};
use axiom_eth::{
    ...
    halo2_base::{
        ...
        halo2_proofs::halo2curves::bn256::Fr, utils::fs::gen_srs,
    },
    ...
    utils::{
        build_utils::pinning::{
            aggregation::AggregationCircuitPinning, Halo2CircuitPinning,
            PinnableCircuit,
        },
        component::{
            promise_loader::{
                comp_loader::SingleComponentLoaderParams,
                multi::MultiPromiseLoaderParams, single::PromiseLoaderParams,
            },
...
use axiom_query::{
    components::{
        ...
        subqueries::{
            account::{
                circuit::{
                    ComponentCircuitAccountSubquery, CoreParamsAccountSubquery,
                },
                types::{
            ...
            block_header::{
                ...
                types::{
                    CircuitInputHeaderShard, CircuitInputHeaderSubquery,
                    ComponentTypeHeaderSubquery, OutputHeaderShard,
                },
            },
    ...
    keygen::shard::{
        ShardIntentAccount, ShardIntentHeader, ShardIntentResultsRoot,
        ShardIntentStorage,
    },
};
...
async fn main() {
    ...
    let storage_pk_path =
        format!("{cargo_manifest_dir}/artifacts/storage_circuit.pk");
    let storage_vk_path =
        format!("{cargo_manifest_dir}/artifacts/storage_circuit.vk");
    let storage_circuit_path =
        format!("{cargo_manifest_dir}/artifacts/storage_circuit.shplonk");
    let account_pinning_path =
        format!("{cargo_manifest_dir}/artifacts/account_circuit_pinning.json");
    let account_pk_path =
        format!("{cargo_manifest_dir}/artifacts/account_circuit.pk");
    let account_vk_path =
        format!("{cargo_manifest_dir}/artifacts/account_circuit.vk");
    let account_circuit_path =
        format!("{cargo_manifest_dir}/artifacts/account_circuit.shplonk");
    let header_pinning_path =
        format!("{cargo_manifest_dir}/artifacts/header_circuit_pinning.json");
    let header_pk_path =
        format!("{cargo_manifest_dir}/artifacts/header_circuit.pk");
    let header_vk_path =
        format!("{cargo_manifest_dir}/artifacts/header_circuit.vk");
    let header_circuit_path =
        format!("{cargo_manifest_dir}/artifacts/header_circuit.shplonk");
    let results_pinning_path =
        format!("{cargo_manifest_dir}/artifacts/results_circuit_pinning.json");
    let results_pk_path =
        format!("{cargo_manifest_dir}/artifacts/results_circuit.pk");
    let results_vk_path =
        format!("{cargo_manifest_dir}/artifacts/results_circuit.vk");
    let results_circuit_path =
        format!("{cargo_manifest_dir}/artifacts/results_circuit.shplonk");
    ...
    let rlc_params = RlcCircuitParams {
        base: base_params,
        num_rlc_columns: NUM_RLC_COLUMNS,
    };
    ...
    log::info!(
        "✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞ before resize header_rlp len, {}",
        header_rlp.len()
    );
    header_rlp.resize(header_rlp_max_bytes, 0_u8);
    log::info!(
        "✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞ after resize header_rlp len, {}",
        header_rlp.len()
    );
    ...
    let (storage_pk, storage_pinning, mut storage_circuit) = {
        ...
        let mut vk_file =
            File::create(&storage_vk_path).expect("strg vk bin file");
        pk.get_vk()
        ...
        let shard_input = Box::new(CircuitInputStorageShard::<Fr> {
            requests: vec![strg_subq_input.clone()],
        ...
        let promises = [
            (
                ...
                ComponentPromiseResultsInMerkle::from_single_shard(
                    generate_keccak_shards_from_calls(
                        &storage_circuit,
                        KECCAK_F_CAPACITY,
                    )
                    .unwrap()
    ...
    let (account_pk, account_pinning, account_circuit) = {
        ...
        let mut vk_file =
            File::create(&account_vk_path).expect("acnt vk bin file");
        account_pk
            .get_vk()
            .write(&mut vk_file, axiom_eth::halo2_proofs::SerdeFormat::RawBytes)
        ...
        let shard_input = Box::new(CircuitInputAccountShard::<Fr> {
        ...
        let promises = [
            ...
            (
                ...
                shard_into_component_promise_results::<
                    Fr,
                    ComponentTypeHeaderSubquery<Fr>,
                >(promise_header.into()),
            ),
        ]
        .into_iter()
    ...
    let (header_pk, header_pinning, header_circuit) = {
        ...
        let (header_pk, header_pinning) = keygen_circuit
            .create_pk(&kzg_params, &header_pk_path, &header_pinning_path)
            .expect("hdr pk and pinning");
        let mut vk_file =
            File::create(&header_vk_path).expect("hdr vk bin file");
        header_pk
            .get_vk()
            .write(&mut vk_file, axiom_eth::halo2_proofs::SerdeFormat::RawBytes)
...
mmr_root: {}",
        ...
        let arr_mmr_proof: [H256; MMR_MAX_NUM_PEAKS - 1] =
            mmr_proof.try_into().expect("mmr proof");
        log::info!(
            "arr_mmr_proof with len {} {:?}",
            &arr_mmr_proof.len(),
            &arr_mmr_proof
        );
        ...
        let arr_mmr_peaks: [H256; MMR_MAX_NUM_PEAKS] =
            mmr_peaks.try_into().expect("mmr peaks");
    ...
    let (results_pk, results_pinning, results_circuit) = {
        ...
        let subquery_hashes: Vec<H256> =
            results.iter().map(|r| r.subquery.keccak()).collect();
        ...
        for (type_id, pr) in component_type_ids.zip_eq([
            ...
            shard_into_component_promise_results::<
                Fr,
                ComponentTypeStorageSubquery<Fr>,
        ...
        let (results_pk, results_pinning) = keygen_circuit
            .create_pk(&kzg_params, &results_pk_path, &results_pinning_path)
            .expect("res pk and pinning");
        let mut vk_file =
            File::create(&results_vk_path).expect("res vk bin file");
        results_pk
            .get_vk()
            .write(&mut vk_file, axiom_eth::halo2_proofs::SerdeFormat::RawBytes)
    ...
    let output_keccak_shard =
        generate_keccak_shards_from_calls(&results_circuit, KECCAK_F_CAPACITY)
            .expect("keccak calls");
    let keccak_merkle =
    ...
    let snark_header = gen_snark_shplonk(
        &kzg_params,
        &header_pk,
        header_circuit,
        Some(&header_circuit_path),
    );
    log::info!("✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞✞ generating results snark");
    let snark_results = gen_snark_shplonk(
        &kzg_params,
        &results_pk,
        results_circuit,
        Some(&results_circuit_path),
    );
    ...
    let subq_aggr_break_points = vec![vec![
        1048565, 1048566, 1048566, 1048566, 1048564, 1048565, 1048566, 1048565,
        1048566, 1048565, 1048564, 1048566, 1048564, 1048566, 1048565, 1048564,
        1048566, 1048566,
    ]];
    let subq_aggr_pinning = AggregationCircuitPinning::new(
        subq_aggr_params,
        subq_aggr_break_points,
    );
    let mut subq_aggr_circuit = InputSubqueryAggregation {
        snark_header: EnhancedSnark {
            inner: snark_header,
            agg_vk_hash_idx: None,
        }, // header needs results
        snark_results_root: EnhancedSnark {
            inner: snark_results,
            agg_vk_hash_idx: None,
        }, // everything needs results root
        snark_account: Some(EnhancedSnark {
            inner: snark_account,
            agg_vk_hash_idx: None,
        }), // account needs header
        snark_storage: Some(EnhancedSnark {
            inner: snark_storage,
            agg_vk_hash_idx: None,
        }), // storage needs account
    ...
    MockProver::run(K as u32, &subq_aggr_circuit, instances)
        .unwrap()
        .assert_satisfied();
