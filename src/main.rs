...
async fn main() {
    ...
    let (header_pk, header_pinning, header_circuit) = {
        ...
        let core_params =
            CoreParamsHeaderSubquery { capacity: HEADER_CAPACITY, max_extra_data_bytes: MAX_EXTRA_DATA_BYTES };
        // let loader_params= PromiseLoaderParams::new_for_one_shard(KECCAK_F_CAPACITY);
        let loader_params =
            PromiseLoaderParams { comp_loader_params: SingleComponentLoaderParams::new(4, vec![KECCAK_F_CAPACITY]) };
